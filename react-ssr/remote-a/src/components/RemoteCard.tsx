...
export default function RemoteCard({ title, children }: { title: string; children?: ReactNode }) {
  return (
    <div style={cardStyle} data-e2e="APP__CARD">
      <CloudIcon />
      <div style={{ marginTop: 10, fontSize: 25 }}>{title}</div>
      <Counter />
      {children}
    </div>
  );
