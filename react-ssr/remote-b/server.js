...
let templateHtml = "";
if (!isProduction) {
  const { createServer, createServerModuleRunner: createRunner } = await import("vite");
  createServerModuleRunner = createRunner;
...
app.get(["/", "/index.html"], async (req, res) => {
  try {
    ...
    if (isProduction) {
      ...
      ssrRunner ??= createServerModuleRunner(vite.environments.ssr);
      ({ render } = await ssrRunner.import(entryServer));
    }
