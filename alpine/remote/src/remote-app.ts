...
const REMOTE_APP_DATA_KEY = "remoteCard";

export function remoteApp(): RemoteAppState {
  return {
    remoteCount: 0,
  };
}
...
export function mountRemoteApp(container: HTMLElement): void {
  Alpine.data(REMOTE_APP_DATA_KEY, remoteApp);

  container.innerHTML = `
    <div x-data="${REMOTE_APP_DATA_KEY}">
      <div class="remote">
        <div class="card">
          <div class="icon">
            <svg
          ...
          <button class="counter" type="button" @click="remoteCount += 1">
            <span x-text="\`Remote counter: \${remoteCount}\`"></span>
          </button>
        </div>
      </div>
    </div>
